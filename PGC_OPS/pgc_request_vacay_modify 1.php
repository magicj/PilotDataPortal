...
if (isset($_GET['vac_key'])) {
  $colname_Recordset1 = (get_magic_quotes_gpc()) ? $_GET['vac_key'] : addslashes($_GET['vac_key']);
}
mysql_select_db($database_PGC, $PGC);
...
$Instuctors = mysql_query($query_Instuctors, $PGC) or die(mysql_error());
$row_Instuctors = mysql_fetch_assoc($Instuctors);
$totalRows_Instuctors = mysql_num_rows($Instuctors);
...
?>
                  ...
                  <?php
} while ($row_Instuctors = mysql_fetch_assoc($Instuctors));
  $rows = mysql_num_rows($Instuctors);
  if($rows > 0) {
      mysql_data_seek($Instuctors, 0);
	  $row_Instuctors = mysql_fetch_assoc($Instuctors);
  }
?>
              </select>
