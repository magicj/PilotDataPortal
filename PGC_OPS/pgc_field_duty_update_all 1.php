...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE pgc_field_duty SET fm=%s, afm1=%s, afm2=%s, afm3=%s, cfig=%s, tp1=%s, tp2=%s,`session`=%s, `delete_record`=%s  WHERE `date`=%s",
...
do {  
?>
                      <option value="<?php echo $row_rsMembers['NAME']?>"<?php if (!(strcmp($row_rsMembers['NAME'], $row_rsFieldDuty['cfig']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsMembers['NAME']?></option>
                      <?php
} while ($row_rsMembers = mysql_fetch_assoc($rsMembers));
  $rows = mysql_num_rows($rsMembers);
  if($rows > 0) {
...
?>
              ...
              <tr valign="baseline">
                  <td align="right" nowrap bgcolor="#CC0000"><div align="left"><em><strong>DELETE:</strong></em></div></td>
                  <td bgcolor="#CC0000"><select name="delete_record" id="delete_record">
