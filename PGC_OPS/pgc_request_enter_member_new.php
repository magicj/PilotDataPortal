...
If (($OffDutyCFIG1 == 'N' AND $OffDutyCFIG2 == 'N' ) OR ($_POST['cfig_confirmed'] == 'YES') ) {	  
  /* UPDATE~RECORD */
	$insertSQL = sprintf("INSERT INTO pgc_request (member_name, member_id, request_date, request_time, request_type, member_weight, request_cfig, request_cfig2, request_notes, cfig_confirmed) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
$totalRows_Requests9 = mysql_num_rows($Requests9);
	...
	$message = $message . $entry_ip . "\n\n";
		...
		$ToList = $ToList . "," . $row_Requests9[cfig2_email];
   	    ...
   	    $webmaster = "kilokilo@comcast.net"; 
		...
		if ($row_System[sys_status] == 'test') {
				$to = "ventusdriver@gmail.com, kilokilo@comcast.net";
		}
...
mysql_select_db($database_PGC, $PGC);
$query_DutyDates = "SELECT date, Date_format(date,'%W, %M %e') as mydate FROM pgc_field_duty WHERE `date` >=CURDATE() AND `date` <= (CURDATE() + Interval 6 Day) ORDER BY `date` ASC LIMIT 3";
...
?>
                                                ...
                                                <tr valign="baseline">
                                                        <td height="1" align="right" valign="middle" nowrap="nowrap" bgcolor="#6666FF" class="style25"><div align="left"><span class="style31">CONFIRMED WITH  CFIG 1:</span></div></td>
                                                        <td height="1" valign="middle" bgcolor="#6666FF"><select name="cfig_confirmed" id="cfig_confirmed">
