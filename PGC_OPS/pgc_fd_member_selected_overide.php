...
//$updateSQL = sprintf("UPDATE pgc_field_duty_selections SET date_selected=%s, modified_date = now() WHERE key_check=%s",
...
$member_session = $row_Recordset1['session']; 
...
if (isset($row_Recordset1['session'])) {
  $colname_Recordset3 = $row_Recordset1['session'];
}
mysql_select_db($database_PGC, $PGC);
$query_Recordset3 = sprintf("SELECT `date`, `session` FROM pgc_field_duty WHERE `session` = %s", GetSQLValueString($colname_Recordset3, "int"));
            ...
            <table width="99%" border="0" cellspacing="2" cellpadding="2">
                <tr>
                      <td bgcolor="#333366"><div align="center">
                            <p class="style24">AFM FIELD DUTY - MEMBER SESSION DUTY DAYS - ADMIN OVER RIDE</p>
                          ...
                          <tr valign="baseline">
                                ...
                                <td width="299" valign="middle" bgcolor="#1C2F5B"><label for="date_selected"></label>
                                      <select name="date_selected" id="date_selected">
                                            <option value="''" <?php if (!(strcmp('', $row_Recordset1['date_selected']))) {echo "selected=\"selected\"";} ?>>No Selection</option>
                                            <?php
do {  
?>
                                            <option value="<?php echo $row_Recordset3['date']?>"<?php if (!(strcmp($row_Recordset3['date'], $row_Recordset1['date_selected']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Recordset3['date']?></option>
                                            <?php
