...
mysql_select_db($database_PGC, $PGC);
$query_DutyDates = "SELECT date, Date_format(date,'%W, %M %e') as mydate FROM pgc_field_duty WHERE fd_type <> 'midweek' AND `date` >=CURDATE() ORDER BY `date` ASC LIMIT 3";
...
$InstructionTypes = mysql_query($query_InstructionTypes, $PGC) or die(mysql_error());
$row_InstructionTypes = mysql_fetch_assoc($InstructionTypes);
$totalRows_InstructionTypes = mysql_num_rows($InstructionTypes);
?>
...
?>
                    ...
                    <?php
} while ($row_InstructionTypes = mysql_fetch_assoc($InstructionTypes));
  $rows = mysql_num_rows($InstructionTypes);
  if($rows > 0) {
      mysql_data_seek($InstructionTypes, 0);
	  $row_InstructionTypes = mysql_fetch_assoc($InstructionTypes);
  }
?>
                  </select></td>
...
mysql_free_result($InstructionTypes);
