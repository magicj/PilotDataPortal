...
/* echo $emaillog;  */
 ...
 $to = $member_email . ", support@pgcsoaring.org, slsnite@comcast.net";
...
if ($_SESSION['S1active'] =='Y') { 
   ...
   If ($_SESSION['MM_duty_role'] == 'FM1' || $_SESSION['MM_duty_role'] == 'FM2' ) {
      $query_fd_datesforsession1 = sprintf("SELECT `date` FROM pgc_field_duty WHERE (fm_count IS NULL OR fm_count < 1) AND `session` = %s ORDER BY `date` ASC", GetSQLValueString('1', "text")); 
   }
   If ($_SESSION['MM_duty_role'] == 'AFM1' || $_SESSION['MM_duty_role'] == 'AFM2' ) {
      $query_fd_datesforsession1 = sprintf("SELECT `date` FROM pgc_field_duty WHERE (afm_count IS NULL OR afm_count < 3 ) AND `session` = %s ORDER BY `date` ASC", GetSQLValueString('1', "text")); 
   }
...
$query_fd_datesforsession2 = sprintf("SELECT `date` FROM pgc_field_duty WHERE `session` = %s ORDER BY `date` ASC", GetSQLValueString('2', "text"));
   ...
   If ($_SESSION['MM_duty_role'] == 'FM1' || $_SESSION['MM_duty_role'] == 'FM2' ) {
      $query_fd_datesforsession2 = sprintf("SELECT `date` FROM pgc_field_duty WHERE (fm_count IS NULL OR fm_count < 1) AND `session` = %s ORDER BY `date` ASC", GetSQLValueString('2', "text")); 
   }
   If ($_SESSION['MM_duty_role'] == 'AFM1' || $_SESSION['MM_duty_role'] == 'AFM2' ) {
      $query_fd_datesforsession2 = sprintf("SELECT `date` FROM pgc_field_duty WHERE (afm_count IS NULL OR afm_count < 3 ) AND `session` = %s ORDER BY `date` ASC", GetSQLValueString('2', "text")); 
   }
...
$query_fd_datesforsession3 = sprintf("SELECT `date` FROM pgc_field_duty WHERE `session` = %s ORDER BY `date` ASC", GetSQLValueString('3', "text"));
   ...
   If ($_SESSION['MM_duty_role'] == 'FM1' || $_SESSION['MM_duty_role'] == 'FM2' ) {
      $query_fd_datesforsession3 = sprintf("SELECT `date` FROM pgc_field_duty WHERE (fm_count IS NULL OR fm_count < 1) AND `session` = %s ORDER BY `date` ASC", GetSQLValueString('3', "text")); 
   }
   If ($_SESSION['MM_duty_role'] == 'AFM1' || $_SESSION['MM_duty_role'] == 'AFM2' ) {
      $query_fd_datesforsession3 = sprintf("SELECT `date` FROM pgc_field_duty WHERE (afm_count IS NULL OR afm_count < 3 ) AND `session` = %s ORDER BY `date` ASC", GetSQLValueString('3', "text")); 
   }
...
<table width="900" border="0" align="center" cellpadding="2" cellspacing="2" bordercolor="#000033" bgcolor="#666666">
  ...
  <tr>
    <td height="681" bgcolor="#3E3E5E"><table width="99%" height="642" border="0" align="center" cellpadding="5" cellspacing="2" bordercolor="#005B5B" bgcolor="#4F5359">
      <tr>
      ...
      <tr>
        <td height="564" align="center" valign="top"><table width="800" align="center" cellpadding="2" cellspacing="2" bgcolor="#6666FF">
              <tr>
                    ...
                    <tr>
                          <td width="694" height="184" valign="middle" bgcolor="#6666FF" class="style32"><div align="center" class="style43">
                                <table width="95%" cellspacing="5" cellpadding="5">
                                      <tr>
                                            <td height="42" align="center" bgcolor="#9CA3ED"><p class="FDtext"> The system is  officially open for field duty date selections.</p>
                                                  <p class="FDtext">For each open session, please select three dates we can use to assign PGC field duty  ... only one choice for each session be used. You will receive an  e-mail documenting your selections.</p></td>
                                      </tr>
                                      <tr>
                                      ...
                                      <tr>
                                            <td height="61" align="center" bgcolor="#9CA3ED"><p class="FDtext">If you don't make a selection - we will make a choice  for you at the end of the session selection period.</p>
                                                  <p class="FDtext">Send an e-mail to PGC Support (support@pgcsoaring.org)  if you have issues or questions.</p></td>
                                      </tr>
                                </table>
