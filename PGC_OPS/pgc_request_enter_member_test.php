...
If (($OffDutyCFIG1 == 'N' AND $OffDutyCFIG2 == 'N' ) OR ($_POST['cfig_confirmed'] == 'YES') ) {	  
  /* UPDATE~RECORD */
	$insertSQL = sprintf("INSERT INTO pgc_request (member_name, member_id, request_date, request_time, request_type, member_weight, request_cfig, request_cfig2, request_notes, cfig_confirmed, sched_assist) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
						   ...
						   GetSQLValueString($_POST['sar'], "text")
						   );
...
mysql_select_db($database_PGC, $PGC);
$query_DutyDates = "SELECT date, Date_format(date,'%W, %M %e') as mydate FROM pgc_field_duty, pgc_date WHERE `date` >=pgc_date.ops_Date AND `date` <= (pgc_date.ops_Date + Interval 6 Day) ORDER BY `date` ASC LIMIT 3";
